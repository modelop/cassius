...
# except ImportError:
#     def readUniTable(*args, **kwds):
#         raise NotImplementedError("Augustus libraries cannot be imported.")
...
class HistogramAbstract(Frame):
    ...
    def __iadd__(self, other):
        if not isinstance(other, HistogramAbstract):
            raise TypeError, "Histograms can only be added to other Histograms"
        if self.bins != other.bins:
            raise TypeError, "Histograms must have identical bins to be added together"
    ...
    def __imul__(self, other):
        if isinstance(other, HistogramAbstract):
            if self.bins != other.bins:
                raise TypeError, "Histograms must have identical bins to be multiplied together"
    ...
    def __idiv__(self, other):
        if isinstance(other, HistogramAbstract):
            if self.bins != other.bins:
                raise TypeError, "Histograms must have identical bins to be divided"
...
class Histogram(HistogramAbstract):
    ...
    Arguments:
       ...
       data (list or `None`): values to send to the histogram's fill
       function upon construction (allows for one-line plotting)

       weights (list or `None`): weights to accompany data (above)
    ...
    def index(self, value):
        """Transform a value into the corresponding bin index."""

        i = int(math.floor((value - self._low)*self._factor))
...
class TimeHist(Histogram):
    """Represent a 1-D histogram over temporal data.

    Arguments:
       numbins (int): number of bins

       low (float): low edge of first bin (suggestion: use the `timesec` function)

       high (float): high edge of last bin (same suggestion)
    ...
    _not_frameargs = Histogram._not_frameargs + ["informat", "outformat"]

    def __init__(self, numbins, low, high, informat="%Y-%m-%d %H:%M:%S", outformat="%Y-%m-%d %H:%M:%S", subseconds=False, t0=0., data=None, weights=None, storelimit=0, linewidth=1., linestyle="solid", linecolor="black", fillcolor=None, gap=0, **frameargs):
        self.informat, self.outformat, self._subseconds, self._t0 = informat, outformat, subseconds, t0
        Histogram.__init__(self, numbins, low, high, data=data, weights=weights, storelimit=storelimit, linewidth=linewidth, linestyle=linestyle, linecolor=linecolor, fillcolor=fillcolor, gap=gap, **frameargs)
    ...
    def fill(self, values, weights=None, limit=None, fillstore=True):
        ...
        Behavior:
           If the `informat` data member is not `None`, values will be
           converted from strings into seconds since epoch.
        ...
        if self.informat is not None:
            values = self.fromtimestring(values)

        return Histogram.fill(self, values, weights=weights, limit=limit, fillstore=fillstore)
...
class HistogramCategorical(HistogramAbstract):
    ...
    def index(self, value):
        ...
        for i, binvalue in enumerate(self.bins):
            if value == binvalue:
                return i
        return None
...
class RegionMap(Frame):
    _not_frameargs = ["xbins", "ybins", "categories", "categorizer", "colors", "bordercolor", "smooth"]

    def __init__(self, xbins, xmin, xmax, ybins, ymin, ymax, categories, categorizer, colors=Auto, bordercolor=None, smooth=False, **frameargs):
        self.xbins, self.xmin, self.xmax, self.ybins, self.ymin, self.ymax, self.categories, self.categorizer, self.colors, self.bordercolor, self.smooth = xbins, xmin, xmax, ybins, ymin, ymax, categories, categorizer, colors, bordercolor, smooth
        Frame.__init__(self, **frameargs)
    ...
    def _prepare(self, xmin=None, ymin=None, xmax=None, ymax=None, xlog=False, ylog=False):
        ...
        if self.colors is Auto:
            colors = color.lightseries(len(self.categories), alternating=False)
        else:
            if len(self.categories) != len(self.colors):
                raise ContainerException, "RegionMap has %d categories, but %d colors" % (len(self.categories), len(self.colors))
            colors = list(self.colors)

        if self.bordercolor is not None:
            colors.append(self.bordercolor)

        ints = [color.RGB(c).ints() for c in colors]

        if hasattr(self, "_cache") and self._cache == id(self._categorizer) and not isinstance(self._categorizer, numpy.ndarray):
            pass

        else:
            if isinstance(self._categorizer, numpy.ndarray):
                values = self._categorizer

            else:
                xstep = (self.xmax - self.xmin)/float(self.xbins)
                ystep = (self.ymax - self.ymin)/float(self.ybins)

                if self.bordercolor is not None:
                    xmin = self.xmin - xstep
                    xmax = self.xmax + xstep
                    ymin = self.ymin - ystep
                    ymax = self.ymax + ystep

                else:
                    xmin, ymin, xmax, ymax = self.xmin, self.ymin, self.xmax, self.ymax

                xcenters = numpy.arange(xmin + xstep/2., xmax, xstep)
                ycenters = numpy.arange(ymin + ystep/2., ymax, ystep)

                values = numpy.frompyfunc(lambda x, y: self.categories.index(self._categorizer(x, y)), 2, 1).outer(xcenters, ycenters)

            if self.bordercolor is not None:
                roll1 = numpy.roll(values,  1, 0)
                roll2 = numpy.roll(values, -1, 0)
                roll3 = numpy.roll(values,  1, 1)
                roll4 = numpy.roll(values, -1, 1)

                mask = numpy.zeros(values.shape, dtype=numpy.bool)
                ...
                numpy.logical_or(mask, numpy.not_equal(values, roll1), mask)
                numpy.logical_or(mask, numpy.not_equal(values, roll2), mask)
                numpy.logical_or(mask, numpy.not_equal(values, roll3), mask)
                numpy.logical_or(mask, numpy.not_equal(values, roll4), mask)

                values[mask] = len(self.categories)

                if not isinstance(self._categorizer, numpy.ndarray):
                    values = values[1:-1,1:-1]

            self._values = [[ints[values[row,col]] for col in xrange(self.ybins)] for row in xrange(self.xbins)]

        # if you change self._categorizer, you'll have to recompute values
        self._cache = id(self._categorizer)
...
def histogram(table, expr, cuts=None, weights=None, bins=10, range=None, categoryToString=True, viewInteractively=True):
    import svgdraw
    import time
    class Callback(object):
        def __init__(self):
            self.lasttime = time.time()
        def __call__(self, hist, bin_edges):
            thistime = time.time()
            if thistime - self.lasttime > 1.:
                svgdraw.view(Histogram(len(bin_edges), bin_edges[0], bin_edges[-1], values=hist))
                self.lasttime = thistime
    if viewInteractively:
        callback = Callback()
    else:
        callback = None

    hist, bin_edges = table.histogram(expr, cuts, weights, bins, range, categoryToString, callback)
    return Histogram(len(bin_edges), bin_edges[0], bin_edges[-1], values=hist)
...
#     def timehist(self, expr, cuts=None, weights=None, informat=None, outformat="%Y-%m-%d %H:%M:%S", subseconds=False, t0=0., numbins=utilities.binning, lowhigh=utilities.calcrange_quartile, use_cache=True, **kwds):
#         """Draw and return a temporal histogram based on the expression and cuts.
...
#         if informat is None and data.dtype.char in numpy.typecodes["Character"] + "SU":
#             informat = "%Y-%m-%d %H:%M:%S"

#         data2 = utilities.fromtimestring(data, informat, subseconds=subseconds, t0=t0)

#         if isinstance(lowhigh, (tuple, list)) and len(lowhigh) == 2 and isinstance(lowhigh[0], (numbers.Number, numpy.number)) and isinstance(lowhigh[1], (numbers.Number, numpy.number)):
#             low, high = lowhigh
#         elif callable(lowhigh):
#             low, high = lowhigh(data2, kwds.get("xlog", False))
#         else:
#             raise ContainerException, "The 'lowhigh' argument must be a function or (low, high) tuple."
...
#             pass
#         elif callable(numbins):
#             numbins = numbins(data2, low, high)
#         else:
#             raise ContainerException, "The 'numbins' argument must be a function or an int."
...
#         kwds2 = {"xlabel": expr}
#         kwds2.update(kwds)
#         output = TimeHist(numbins, low, high, None, outformat, subseconds, t0, data2, dataweight, **kwds2)
...
#     allfiles = []
#     for f in files:
#         # drop duplicates, but maintain order
#         newfiles = [i for i in glob.glob(f) if i not in allfiles]
#         allfiles.extend(newfiles)

#     if len(allfiles) == 0:
#         raise IOError, "No such files: %s" % repr(files)
