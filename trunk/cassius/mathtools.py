...
def principleComponents(values):
    """Return the two largest-variance components of an orthogonal
    transformation that minimizes covariance in the data (PCA).

    Arguments:

       values (list of N-dimensional arrays; can be numpy): the
       dataset for which to find an optimal transformation

    TODO:

       Take a parameter N and return the N largest-variance components.
    """

    values = numpy.matrix(values)
    origin = numpy.mean(values, axis=0)
    values -= origin

    eigenvalues, eigenvectors = numpy.linalg.eig(numpy.cov(values.T))
    index = numpy.argsort(eigenvalues)[::-1]  # first is largest

    origin = origin.tolist()[0]
    xscale = math.sqrt(eigenvalues[index[0]])
    xbasis = eigenvectors[:,index[0]].tolist()
    yscale = math.sqrt(eigenvalues[index[1]])
    ybasis = eigenvectors[:,index[1]].tolist()

    dropComplex = lambda x: complex(x).real

    return origin, xscale, map(dropComplex, xbasis), yscale, map(dropComplex, ybasis)
