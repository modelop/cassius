...
def view(obj, **kwds):
    ...
    if hasattr(obj, "_drawClass"):
        drawClass = obj._drawClass
    else:
        drawClass = obj.__class__

    try:
        subcommand = eval("_draw_%s" % drawClass.__name__)
    except NameError:
        raise NotImplementedError, "A '_draw_%s' function has not been implemented in backends.svg" % drawClass.__name__
    ...
    def recurse(plotsin, nonlegends, legends):
        for plot in plotsin:
            if isinstance(plot, containers.Stack):
                if not hasattr(plot, "_overlay"): plot.overlay()
...
def _draw_TimeHist(obj, **kwds):
    _draw_Histogram(obj, **kwds)

def _frameargs_prehook_TimeHist(obj, output, **kwds):
    if "ymin" not in output or output["ymin"] is None or output["ymin"] is containers.Auto:
        if "ylog" not in output or not output["ylog"]:
            output["ymin"] = 0.
...
def _frameargs_posthook_TimeHist(obj, output, **kwds):
    f = obj._frameargs()
...
def _makemarker(shape, markersize, plotmarkname, markeroutline, markeroutlineopacity, markercolor, markeropacity):
    if shape == "circle":
        radius = markersize
        return u"""    <circle id="%(plotmarkname)s" cx="0" cy="0" r="%(radius)g" stroke="%(markeroutline)s" stroke-opacity="%(markeroutlineopacity)g" fill="%(markercolor)s" fill-opacity="%(markeropacity)g" />""" % vars()
    elif shape == "square":
        hp = markersize
        hm = -markersize
        return u"""    <path id="%(plotmarkname)s" d="M %(hm)g,%(hm)g L %(hp)g,%(hm)g L %(hp)g,%(hp)g L %(hm)g,%(hp)g z" stroke="%(markeroutline)s" stroke-opacity="%(markeroutlineopacity)g" fill="%(markercolor)s" fill-opacity="%(markeropacity)g" />""" % vars()
    elif shape == "diamond":
        hp = math.sqrt(2.)*markersize
        hm = -math.sqrt(2.)*markersize
        return u"""    <path id="%(plotmarkname)s" d="M %(hm)g,0 L 0,%(hm)g L %(hp)g,0 L 0,%(hp)g z" stroke="%(markeroutline)s" stroke-opacity="%(markeroutlineopacity)g" fill="%(markercolor)s" fill-opacity="%(markeropacity)g" />""" % vars()
    elif shape == "plus":
        hp = markersize
        hm = -markersize
        return u"""    <path id="%(plotmarkname)s" d="M %(hm)g,0 L %(hp)g,0 M 0,%(hm)g L 0,%(hp)g" stroke="%(markercolor)s" stroke-opacity="%(markeropacity)g" />""" % vars()
    elif shape == "times":
        hp = markersize/math.sqrt(2.)
        hm = -markersize/math.sqrt(2.)
        return u"""    <path id="%(plotmarkname)s" d="M %(hm)g,%(hm)g L %(hp)g,%(hp)g M %(hp)g,%(hm)g L %(hm)g,%(hp)g" stroke="%(markercolor)s" stroke-opacity="%(markeropacity)g" />""" % vars()
    else:
        return None
...
def _draw_Scatter(obj, **kwds):
    ...
    theMarker = _makemarker(obj.marker, _svgmarkersize(obj.markersize), plotmarkname, markeroutline, markeroutlineopacity, markercolor, markeropacity)
    if theMarker is not None:
        svg.defs[plotmarkname] = theMarker
        ...
        for j in range(len(obj._fields[i])):
            ...
            if might_have_style:
                ...
                if "marker" in drawable.__dict__ and drawable.marker is not None:
                    ...
                    markersize, markeroutlineopacity, markeroutline, markeropacity, markercolor = _svgmarkersize(drawable.markersize), _svgopacity(drawable.markeroutline), _svgcolor(drawable.markeroutline), _svgopacity(drawable.markercolor), _svgcolor(drawable.markercolor)
                    theMarker = _makemarker(drawable.marker, markersize, plotmarkname, markeroutline, markeroutlineopacity, markercolor, markeropacity)
                    if theMarker is not None:
                        svg.defs[plotmarkname] = theMarker
