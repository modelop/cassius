...
class RGB(AbstractColor):
    ...
    def __hash__(self):
        return ("RGB", self.r, self.g, self.b, self.opacity).__hash__()
...
class HLS(AbstractColor):
    ...
    def __hash__(self):
        return ("HLS", self.h, self.l, self.s, self.opacity).__hash__()
...
class HSV(AbstractColor):
    ...
    def __hash__(self):
        return ("HSV", self.h, self.s, self.v, self.opacity).__hash__()
...
def truncated_shift(value, shift):
    """Return the sum of value and shift, but stop at the border of the unit interval."""
...
def asymptotic_shift(value, shift):
    """Return the approximate sum of value and shift, in a way that limits to the border of the unit interval.

    The unit interval is mapped by arctangent to the real number line,
    shift is applied, and then the result is transformed back to the
    unit interval.  The mapping is approximately linear near 0.5.
    """
...
def lighten(color, amount=1., method=asymptotic_shift):
    """Return a lighter shade of the input color.

    Arguments:
       color (string or color object): input color

       amount (number): distance in the unit interval to increase lightness

       method (function): how to treat the borders of the unit interval
    """
...
def darken(color, amount=1., method=asymptotic_shift):
    ...
    Arguments:
       ...
       amount (number): distance in the unit interval to decrease lightness
...
def gradient(stops, colors, name=None):
    """Return a function that maps an interval to a smooth gradient of colors.

    Arguments:
       stops (array of numbers): points along the unit interval where
       colors are specified

       colors (array of colors, same length): color at each stop

       name (string or `None`): optional name for the returned function

    Return value:
       A function f(value, low, high), where value must be between low
       and high.

       This function returns 4-tuples (R, G, B, opacity) as integers
       between 0 and 255, for faster conversion into bitmap plots.

       .. todo:: There are still some performance issues; some of
                 these 4-tuples return floats, which is part of the
                 problem.
    """
    ...
    if isinstance(colors[0], RGB):
        fastcolors = numpy.empty((len(colors), 4), dtype=int)
        for i in xrange(len(colors)):
            ...
            fastcolors[i] = (int(math.floor(r)), int(math.floor(g)), int(math.floor(b)), int(math.floor(a)))
...
def darkseries(N, alternating=True, phase=0.05):
    """Return a sequence of N different dark colors (maximally different hues).

    Arguments:
       N (int): number of colors

       alternating (bool): if False, colors are returned in rainbow
       order; if True, colors are alternated to maximize hue
       differences between neighbors

       phase (number): starting phase in the hue unit interval (only
       important for small numbers of colors)
    """
...
def lightseries(N, alternating=True, phase=0.17):
    """Return a sequence of N different light colors (maximally different hues).
